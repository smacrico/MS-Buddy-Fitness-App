...
db_path = st.text_input(
    "Database Path",
    value=r"C:\Users\XP222SP\myHealthData\DBs\garmin_activities.db"
)
...
QUERIES = {
    ...
    "heart_rate_drift": '''
        ...
        SELECT r.activity_id,
               ...
               (AVG(CASE WHEN r.timestamp > q.start_ts + 0.75 * (q.end_ts - q.start_ts) THEN r.hr END) -
                AVG(CASE WHEN r.timestamp < 0.25 * (q.end_ts - q.start_ts) THEN r.hr END)) AS hr_drift
    ...
    ''',
    "activities_list": '''
        SELECT activity_id, sport, start_time, distance, elapsed_time , avg_hr
        FROM activities
