...
class HRVAnalyticsV33:
    def __init__(seLF, db_path="c:/smakrykoDBs/Artemis.db"):
        ...
        seLF.output_dir = "C:/temp/logsFitnessApp/HRV_DashBoards"
    ...
    def plot_time_trends(seLF, df, title="HRV Metrics Time Trends"):
        ...
        filename = os.path.join(seLF.output_dir, f'HRV_TimeTrends_{timestamp}.png')
    ...
    def plot_hrv_histograms(seLF, df, title="HRV Metrics Distributions"):
        ...
        plt.tight_layout()
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = os.path.join(seLF.output_dir, f'HRV_Histograms_{timestamp}.png')
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        logger.info(f"Saved plot: {filename}")
        plt.show()
    ...
    def plot_baseline_bar(seLF, baselines, title="90-Day HRV Baseline Profile"):
        ...
        plt.tight_layout()
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = os.path.join(seLF.output_dir, f'HRV_BaselineProfile_{timestamp}.png')
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        logger.info(f"Saved plot: {filename}")
        plt.show()
    ...
    def plot_radar_chart(seLF, latest, baselines, title="Latest vs Baseline (Radar)"):
        ...
        plt.tight_layout()
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = os.path.join(seLF.output_dir, f'HRV_RadarChart_{timestamp}.png')
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        logger.info(f"Saved plot: {filename}")
        plt.show()
    ...
    def plot_ms_score(seLF, df, title="MS-Aware HRV Health Score (Trend)"):
        ...
        plt.tight_layout()
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = os.path.join(seLF.output_dir, f'HRV_MSScore_{timestamp}.png')
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        logger.info(f"Saved plot: {filename}")
        plt.show()
    ...
    def plot_trend_summary(seLF, stats, title="Trend Statistics Summary"):
        ...
        plt.tight_layout()
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = os.path.join(seLF.output_dir, f'HRV_TrendSummary_{timestamp}.png')
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        logger.info(f"Saved plot: {filename}")
        plt.show()
