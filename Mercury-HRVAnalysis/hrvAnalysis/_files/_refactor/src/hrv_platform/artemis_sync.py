...
from .api.routes import build_summary
...
class ArtemisSyncService:
    ...
    def sync(self, source_name: str | None = None) -> ArtemisSyncResult:
        ...
        rows = self.source.normalize(read_result.dataframe, default_source_name=source_name)
    ...
    async def sync_and_publish(self, source_name: str | None = None) -> ArtemisSyncResult:
        ...
        summary = build_summary(source_name=source_name or settings.source_name_default)
    ...
    async def watch_forever(self, interval_seconds: int | None = None, source_name: str | None = None) -> None:
        ...
        while True:
            try:
                ...
                await event_bus.publish({"type": "artemis_sync_error", "payload": {"message": str(exc)}})
