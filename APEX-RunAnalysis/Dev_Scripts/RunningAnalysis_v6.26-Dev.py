...
class RunningAnalysis:
    ...
    # Define output directory for all charts
    CHART_OUTPUT_DIR = 'c:/temp/logsFitnessApp/Run_DashBoards'
            ...
            for year_month in monthly_avg.index:
                ...
                cursor.execute("""
                    ...
                    ON CONFLICT(year_month) DO UPDATE SET
                        ...
                        energy_cost_std=excluded.energy_cost.std,
                        trimp_mean=excluded.trimp_mean,
                        trimp_std=excluded.trimp.std,
                        recovery_score_mean=excluded.recovery_score_mean,
                        recovery_score_std=excluded.recovery_score.std,
                        readiness_score_mean=excluded.readiness_score_mean,
                        readiness_score_std=excluded.readiness_score.std,
                        avg_speed_mean=excluded.avg_speed_mean,
                        avg_speed_std=excluded.avg_speed.std,
                        max_speed_mean=excluded.max_speed_mean,
                        max_speed_std=excluded.max_speed.std,
                        speed_reserve_mean=excluded.speed_reserve_mean,
                        speed_reserve_std=excluded.speed_reserve.std,
                        hr_rs_deviation_mean=excluded.hr_rs_deviation_mean,
                        hr_rs_deviation_std=excluded.hr_rs_deviation.std,
                        speed_efficiency_mean=excluded.speed_efficiency_mean,
                        speed_efficiency_std=excluded.speed_efficiency.std
                """, (
                    str(year_month),
                    sessions,
    ...
    def visualize_training_load(self):
        ...
        try:
            ...
            plt.tight_layout()
            output_path = os.path.join(self.CHART_OUTPUT_DIR, f'Run_TrainingLoad_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
            plt.savefig(output_path, dpi=300, bbox_inches='tight')
            print(f"Training load chart saved to: {output_path}")
            plt.show()
    ...
    def visualize_trends(self):
        ...
        try:
            ...
            plt.tight_layout()
            output_path = os.path.join(self.CHART_OUTPUT_DIR, f'Run_Trends_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
            plt.savefig(output_path, dpi=300, bbox_inches='tight')
            print(f"Trends chart saved to: {output_path}")
            plt.show()
        except Exception as e:
    ...
    def visualize_recovery_and_readiness(self):
        ...
        plt.legend()
        plt.tight_layout()
        output_path = os.path.join(self.CHART_OUTPUT_DIR, f'Run_RecoveryReadiness_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Recovery and readiness chart saved to: {output_path}")
        plt.show()
    ...
    def advanced_visualizations(self):
       ...
       # In the advanced_visualizations method, modify the pie chart section:
        ...
        plt.tight_layout()
        output_path = os.path.join(self.CHART_OUTPUT_DIR, f'Run_AdvancedMetrics_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Advanced metrics chart saved to: {output_path}")
        plt.show()
    ...
    def visualize_score_impact_over_time(self, extra_scores=None):
        ...
        plt.legend()
        plt.tight_layout()
        output_path = os.path.join(self.CHART_OUTPUT_DIR, f'Run_ScoreImpact_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Score impact chart saved to: {output_path}")
        plt.show()
    ...
    def visualize_speed_metrics(self):
        ...
        try:
            ...
            plt.tight_layout()
            output_path = os.path.join(self.CHART_OUTPUT_DIR, f'Run_SpeedMetrics_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
            plt.savefig(output_path, dpi=300, bbox_inches='tight')
            print(f"Speed metrics chart saved to: {output_path}")
            plt.show()
    ...
    def visualize_hr_rs_deviation(self):
        ...
        try:
            ...
            plt.tight_layout()
            output_path = os.path.join(self.CHART_OUTPUT_DIR, f'Run_HRRSDeviation_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
            plt.savefig(output_path, dpi=300, bbox_inches='tight')
            print(f"HR-RS deviation chart saved to: {output_path}")
            plt.show()
    ...
    def create_performance_dashboard(self):
        ...
        try:
            ...
            plt.savefig(os.path.join(self.CHART_OUTPUT_DIR, f'Run_PerformanceDashboard_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'), 
                       dpi=300, bbox_inches='tight')
            plt.show()
            
            print(f"\nDashboard saved to: {self.CHART_OUTPUT_DIR}")
